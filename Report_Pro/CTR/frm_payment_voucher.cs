    ...
    public partial class frm_payment_voucher : frm_Master
    {
        ...
        private void print_sand_Click(object sender, EventArgs e)
        {
            ...
            if (Properties.Settings.Default.lungh == "0")
            {
                ...
                dt1 = dal.getDataTabl_1(@"select A.*,B.*,P.PAYER_NAME,C.branch_name from (SELECT LEFT(D.BRANCH_code,1) as com_code,ACC_YEAR,D.ACC_NO,D.BRANCH_code,PAYER_NAME as PAYER_NAME_Db,PAYER_l_NAME as PAYER_L_NAME_Db ,ser_no,D.COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
                    ,user_name,desc2,sheek_no,sheek_bank,sheek_date,sheek_or_cash,D.notes,SOURCE_CODE,Wh_Branch_Code,Sheek,payType_No,shequeBank_no,ISNULL(NULLIF(desc_E, ''), desc2) as desE
                    ,Dafter_no,Dafter_ser,SANAD_TYPE2  FROM daily_transaction as D inner join payer2 as P_Db on P_Db.ACC_NO=D.ACC_NO and P_Db.BRANCH_code=D.BRANCH_code   where SANAD_TYPE2='" + txt_sanad_type2.Text + "' and D.BRANCH_code='" + txtStore_ID.Text + "' and  ser_no='" + AccSer_No.TextS + "' and  meno>0) as A " +
            ...
            else
            {
                ...
                dt1 = dal.getDataTabl_1(@"select A.*,B.*,P.PAYER_NAME,p.PAYER_l_NAME,C.branch_name from (SELECT LEFT(D.BRANCH_code,1) as com_code,ACC_YEAR,D.ACC_NO,D.BRANCH_code,PAYER_NAME as PAYER_NAME_Db,PAYER_l_NAME as PAYER_L_NAME_Db ,ser_no,D.COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
                    ,user_name,desc2,sheek_no,sheek_bank,sheek_date,sheek_or_cash,D.notes,SOURCE_CODE,Wh_Branch_Code,Sheek,payType_No,shequeBank_no,ISNULL(NULLIF(desc_E, ''), desc2) as desE
                    ,Dafter_no,Dafter_ser,SANAD_TYPE2  FROM daily_transaction as D inner join payer2 as P_Db on P_Db.ACC_NO=D.ACC_NO and P_Db.BRANCH_code=D.BRANCH_code   where SANAD_TYPE2='" + txt_sanad_type2.Text + "' and D.BRANCH_code='" + txtStore_ID.Text + "' and  ser_no='" + AccSer_No.TextS + "' and  meno>0) as A " +
                "inner join wh_BRANCHES as C on C.Branch_code=A.Branch_code " +
                ",(select acc_no as acc_cr,desc2 as desc_cr  from daily_transaction where SANAD_TYPE2='" + txt_sanad_type2.Text + "' and BRANCH_code='" + txtStore_ID.Text + "' and  ser_no='" + AccSer_No.TextS + "' and loh>0)  as B " +
        ...
        public override void Print()
        {
            ...
            CTR.rpt_Payment rpt = new CTR.rpt_Payment();
            DataTable dt1 = new DataTable();
            DataSet ds = new DataSet();

            dt1 = dal.getDataTabl_1(@"select A.*,B.*,P.PAYER_NAME,P.PAYER_l_NAME,C.branch_name,C.WH_E_NAME,S.BANK_NAME,S.BANK_NAME_E from (SELECT LEFT(D.BRANCH_code,1) as com_code,ACC_YEAR,D.ACC_NO,D.BRANCH_code,PAYER_NAME as PAYER_NAME_Db,PAYER_l_NAME as PAYER_L_NAME_Db ,ser_no,D.COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
                    ,user_name,desc2,sheek_no,sheek_bank,sheek_date,sheek_or_cash,D.notes,SOURCE_CODE,Wh_Branch_Code,Sheek,payType_No,shequeBank_no,ISNULL(NULLIF(desc_E, ''), desc2) as desE
                    ,Dafter_no,Dafter_ser,SANAD_TYPE2  FROM daily_transaction as D inner join payer2 as P_Db on P_Db.ACC_NO=D.ACC_NO and P_Db.BRANCH_code=D.BRANCH_code where SANAD_TYPE2='" + txt_sanad_type2.Text + "' and D.BRANCH_code='" + txtStore_ID.Text + "' and  ser_no='" + AccSer_No.TextS + "' and  meno>0) as A " +
                   "inner join wh_BRANCHES as C on C.Branch_code=A.Branch_code " +
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                decimal balance_ = Convert.ToDecimal(dt1.Rows[0]["meno"].ToString());
                ToWord toWord = new ToWord(Math.Abs(Math.Round(balance_, dal.digits_)), currencies[currencyNo]);
                rpt.txtBranch.Text = dt1.Rows[0]["branch_name"].ToString();
                rpt.txtBranch_E.Text = dt1.Rows[0]["WH_E_NAME"].ToString();


                if (Properties.Settings.Default.lungh == "0")
                {
                    rpt.txtAmountinLetter.Text = "'" + toWord.ConvertToArabic().ToString() + "'";
                    rpt.txtAccount.Text = dt1.Rows[0]["PAYER_NAME_Db"].ToString();
                    rpt.txtChequeBank.Text = dt1.Rows[0]["BANK_NAME"].ToString();
                    rpt.txtDesc1.Text = dt1.Rows[0]["desc2"].ToString();
                    rpt.txtDepositAcc.Text = dt1.Rows[0]["PAYER_NAME"].ToString();
                }
                else
                {
                    rpt.txtAmountinLetter.Text = "'" + toWord.ConvertToEnglish().ToString() + "'";
                    rpt.txtAccount.Text = dt1.Rows[0]["PAYER_l_NAME_Db"].ToString();
                    rpt.txtChequeBank.Text = dt1.Rows[0]["BANK_NAME_E"].ToString();
                    rpt.txtDesc1.Text = dt1.Rows[0]["desE"].ToString();
                    rpt.txtDepositAcc.Text = dt1.Rows[0]["PAYER_L_NAME"].ToString();
                }
