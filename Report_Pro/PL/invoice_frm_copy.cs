...
getSalesInv(Sh_ser, txtStore_ID.Text, sh_transaction, Properties.Settings.Default.C_year);
        ...
        {
            dt_inv = this.dal.getDataTabl_1(@"select A.ser_no,A.Sanad_no,A.Branch_code,A.Cyear,A.Transaction_code,
            A.QUOT_REF_NO,A.QUOT_REF_DATE,A.SO_no,A.DelevryE,A.TermsOfPaymentE,BankNo,
            ...
            ,B.modular,B.DP_Ratio,B.DP_Amount,B.VAT_RATIO,B.main_counter,B.DETAILS,B.ITEM_NO,B.QTY_ADD,B.QTY_TAKE,
            B.Unit,B.Local_Price,isnull(B.TAX_IN,0)as TAX_IN ,isnull(B.TAX_OUT,0)as TAX_OUT , round(B.total_disc*B.local_price*QTY_TAKE/100,2) as disc_ ,B.Pice_Total_Cost,b.balance,B.K_M_TYPE_ITEMS,B.DeleveryNote,
            ...
            //select A.ser_no,A.Sanad_no,A.Branch_code,A.Cyear,A.Transaction_code,A.G_date,A.Acc_no,A.Payment_Type,A.Sales_man_Id,A.Inv_no,A.Inv_date,a.Inv_Notes,A.Phone,A.Adress,A.PROJECT_NO,A.Costmer_No,A.Cash_costomer_name,A.REQUIST_NO,A.Ref
        ...
        public override void Print()
        {
           ...
           ds.WriteXmlSchema("schema_rpt.xml");
                ...
                RPT.Rpt_DownPayment_Invoice reportInv = new RPT.Rpt_DownPayment_Invoice();
        ...
        private void PaymentTearms_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        public override void Save()
        {
            ...
            try
            {
                ...
                if (txt_downPayment_Acc.ID.Text.Trim() == "" || txt_CashAcc.ID.Text.Trim() == "" || txt_vatAcc.ID.Text.Trim() == "" || txtCostOfSales.ID.Text.Trim() == "" || txtInventory.ID.Text.Trim() == "")
                {
            ...
            values('" + txt_InvNu.Text +
                ...
                if (Properties.Settings.Default.TRANS_TO_ACC == "A")
                {
                    ...
                    if (txt_PayType.ID.Text == "2")
                    {
                        ...
                        arabicDesc = "فاتورةدفعة مقدمةآجل رقم " + txt_InvNu.Text + (Po_No.Text.Trim() != "" ? " طلب شراء " + Po_No.Text : "");
                        englishDesc = "Credit Down Payment Invoice No " + txt_InvNu.Text + (Po_No.Text.Trim() != "" ? " PO " + Po_No.Text : "");
                    }
                    else
                    {
                        arabicDesc = "فاتورةدفعة مقدمة نقدي رقم " + txt_InvNu.Text + (Po_No.Text.Trim() != "" ? " طلب شراء " + Po_No.Text : "");
                        englishDesc = "Cash Down Payment Invoice No " + txt_InvNu.Text + (Po_No.Text.Trim() != "" ? " PO " + Po_No.Text : "");
                    }
                    ...
                    cmd.CommandText = @"INSERT INTO daily_transaction ( ACC_YEAR, ACC_NO, BRANCH_code, ser_no, COST_CENTER, meno, loh,
                    balance, h_date,g_date,sanad_no, SANAD_TYPE, sp_ser_no, user_name, desc2,POASTING, SOURCE_CODE, Wh_Branch_Code, MAIN_SER_NO,desc_E )
                    Values('" + acc_year.Text + "', '" + txt_downPayment_Acc.ID.Text + "', '" + txtBranch_Id.Text + "','" + AccSer_No.Text + "','" +
                    Uc_Cost.ID.Text + "','" + 0 + "','" + invGrid1.txtNetTotal.Text.ToDecimal() + "','" + -invGrid1.txtNetTotal.Text.ToDecimal() + "','" + H_Date + "','" +
                    txt_InvDate.Value.ToString("yyyy-MM-dd HH:mm:ss") + "','" + txt_InvNu.Text + "','" + Doc_Type.Text + "','" + Doc_Type.Text + txt_InvNu.Text + "','" +
                    userID.Text + "','" + arabicDesc + "','0','" + Doc_Type.Text + "','" +
                    txtStore_ID.Text + "','" + AccSer_No.Text + "','" + englishDesc + "')";
