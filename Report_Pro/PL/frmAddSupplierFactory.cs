...
{
    public partial class frmAddSupplierFactory :Form
    {
        ...
        private void getSer()
        {
            txtCode.Text = dal.GetCell_1(@"SELECT isnull(max(id)+1,1) as ser FROM SuppFactoreis").ToString();
        }
        ...
        {
            DataTable dt_ = dal.getDataTabl_1("SELECT id,Name,NameE FROM SuppFactoreis where id like '%" + txtCode.Text + "%' and Name like '%" + txtDesc.Text + "%' and NameE like '%" + txtDesc_L.Text + "%'");
            dt.Clear();
            int i = 0;
            DataRow row = null;
        ...
        private void fillDG()
        {
            DataTable dt_ = dal.getDataTabl_1("SELECT id,Name,NameE FROM SuppFactoreis");
            dt.Clear();
            int i = 0;
            DataRow row = null;
            foreach (DataRow r in dt_.Rows)
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (txtCode.Text != string.Empty && txtDesc.Text != string.Empty)
                ...
                {
                    {
                        dal.Execute_1("insert into  SuppFactoreis (id,Name,NameE) values('" + txtCode.Text + "','" + txtDesc.Text + "','" + txtDesc_L.Text + "')");
                ...
                {
                    dal.Execute_1("update  SuppFactoreis set Name ='" + txtDesc.Text + "',NameE ='" + txtDesc_L.Text + "' where id = '" + txtCode.Text + "'");
