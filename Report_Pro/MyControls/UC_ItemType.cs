...
{
    public partial class UC_ItemType : UserControl
    {
        ...
        private void get_desc()
        {
            try
            {
                ...
                {
                    ID.Text= dt_.Rows[0]["class_Code"].ToString();
        ...
        private void search_()
        {
            try
            {
                ...
                if (Properties.Settings.Default.lungh == "0")
                {
                    dgv1.DataSource = dal.getDataTabl_1("SELECT class_Code,descr  FROM wh_class where descr like '%" + Desc.Text + "%' or Descr_eng like '%" + Desc.Text + "%' ");
                }
                else
                {
                    dgv1.DataSource = dal.getDataTabl_1("SELECT class_Code,Descr_eng  FROM wh_class where Descr_eng like '%" + Desc.Text + "%' or Descr_eng like '%" + Desc.Text + "%' ");
