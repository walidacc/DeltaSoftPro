...
{
    public partial class UC_SalesGroup : UserControl
    {
        DAL.DataAccesslayer1 dal = new DAL.DataAccesslayer1();
        public UC_SalesGroup()
        {
            InitializeComponent();
        }
        ...
        private void get_desc()
        {
            try
            {
                DataTable dt_ = dal.getDataTabl_1("SELECT  Code,Descr,Descr_E FROM Price_Group_Table where code = '" + ID.Text + "' and t_final like '" + txtFinal.Text+"%'");
                if (ID.Text!="" && dt_.Rows.Count > 0)
                {
                    ID.Text= dt_.Rows[0]["Code"].ToString();
        ...
        private void search_()
        {
            try
            {
                ...
                if (Properties.Settings.Default.lungh == "0")
                {
                    dgv1.DataSource = dal.getDataTabl_1("SELECT  Code,Descr FROM Price_Group_Table where (Descr like '%" + Desc.Text + "'+'%' or  Descr_E like '%" + Desc.Text + "'+'%') and t_final like '" + txtFinal.Text + "%' ");
                }
                else
                {
                    dgv1.DataSource = dal.getDataTabl_1("SELECT  Code,Descr_E FROM Price_Group_Table where (Descr like '%" + Desc.Text + "'+'%' or  Descr_E like '%" + Desc.Text + "'+'%') and t_final like '" + txtFinal.Text + "%' ");
                }
