    ...
    public partial class UC_KM_Sales : UserControl
    {
        ...
        private void search_()
        {
            try
            {
                ...
                this.BringToFront();
                dgv1.Visible = true;
                dgv1.DataSource = dal.getDataTabl_1("SELECT * FROM KM_MAIN_ACC_CODE where MAIN_KM_CODE like '1%' and MAIN_KM_DESC like'%" + Desc.Text + "%'");
        ...
        private void get_desc()
        {
            try
            {
                DataTable dt_ = dal.getDataTabl_1("SELECT MAIN_KM_DESC FROM KM_MAIN_ACC_CODE  where MAIN_KM_CODE like '1%' and MAIN_KM_CODE = '" + ID.Text + "'");
                if (dt_.Rows.Count > 0)
                {
